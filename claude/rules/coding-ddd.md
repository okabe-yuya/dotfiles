...
- 自身のバリデーションは生成時に行い、不正な状態のValue Objectが存在できないようにすること
    - プリミティブ（String等）を無検査で受け取らず、値オブジェクトの生成時検証（ファクトリ + `require`）で不正インスタンスを作れなくすること
    - 命名規約やパス構造など「実装上の慣習」に依存する検証は、絶対的な保証ではなく best-effort な制約である旨と、その狙い（例: 越境参照のブロック）をコメントで明示すること
