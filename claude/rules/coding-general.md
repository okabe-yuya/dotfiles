...
- レビュー指摘への対応・作業経緯などの「言い訳/説明」はコメントに書かないこと。コメントは設計意図のみ
- 冗長なコメントは避け簡潔にすること。設定値・引数の理由コメントは、その対象の直上の行に置くこと
...
- 不要な依存を作らないこと。特定ベンダー・実装詳細に対する制約を、それを扱うべきでない層に持ち込まないこと
    - 例: 印刷指示を組み立てる層でストレージのスキーム（`gs://`）を必須検証すると、S3 移行時にここがボトルネックになる。スキームの制約はストレージを扱う層に閉じる
- 既存実装のパターン・慣習に合わせること。命名だけでなく、エラー処理・観測性（Span/ログの張り方、Span を呼び出し側で生成し関数は `Span.current()` を参照する等）・相関ID の採番方針まで、まず既存の実装（例: 類似 UseCase）がどうしているかを確認し、独自流儀を持ち込まない
- デメテルの法則に従うこと
    - 詳細はdddに関する別ルールを参照
