...
- 5行以上の実装を持つ関数を式本体（`fun foo() = …`）で書いていないか。ブロック本体（`fun foo() { … }`）で書く（短い1行関数は式本体でよい）
- 例外の扱いが適切か: catch した経路で失敗が結果に反映されず画面上「成功」と表示されていないか。ハンドリングの責務が呼び出し側にある場合は下層で抱え込まず上位へ委ねる（型で失敗を返す／例外を伝播）
...
- 新しく DataLoader／Resolver 等の共通機構を定義する前に、既存の同種の共通機構を再利用できないか（既存 DataLoader 経由で必要なフィールドだけ引く）
...
- コメントが長すぎ／冗長でないか（step の逐次説明や自明な詳細を削り、順序保証の目的など要点だけ残す）
- TODOにタイムスタンプがあるか
...
- 入力（GraphQL input・関数引数・データクラス）に安易な default 値／default 引数を付けていないか。想定しづらいケースを見越して nullable・default にせず、常に値が要るなら必須にする（default 値・default null の多用は避け、既定に流れて意図が隠れるのを防ぐ）。ただし既にデプロイ済みのクライアントが依存するフィールドは後方互換のため nullable 自体は残し、外すのは `= null` の既定だけにする
- ロジックをほとんど持たない局所的な組み合わせを、わざわざ専用データクラスにしていないか（`Pair` 等で足りる）。逆にドメインの意味を持つ値・公開層で扱う値は専用型にする（primitive/Pair の乱用と専用型化は文脈で使い分ける）
...
- 公開済みフィールドの削除は二段階リリースで行う（まず `@Deprecated` を付け、後方互換のため型は nullable のまま残す → クライアント移行後の別リリースで削除）。`@Deprecated` はスキーマ公開層のフィールド（GraphQL schema／Mutation input／Resolver の公開フィールド）に付け、内部層（RepositoryImpl・DAO 等）には付けない
